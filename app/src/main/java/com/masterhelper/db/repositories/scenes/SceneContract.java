...
public class SceneContract extends AbstractContract<SceneModel> {
  ...
  public SceneContract(DbHelpers dbHelpers) {
    super(dbHelpers);
    initContract(TABLE_NAME, new GeneralColumn[]{id, title, description});
  ...
  public void insertRecord(SceneModel record) {
    String[] values = new String[]{record.id.get().toString(), record.name.get(), record.description.get(), this.journeyId};
